...
app.use((req, res, next) => {
  ...
  const hostStr = typeof host === 'string' ? host.toLowerCase().trim() : '';

  let protocol = req.headers['x-forwarded-proto'] || req.protocol || 'http';
  if (Array.isArray(protocol)) {
    protocol = protocol[0];
  }
  const protocolStr = typeof protocol === 'string' ? protocol.toLowerCase().trim() : 'http';

  let shouldRedirect = false;
  let newHost = hostStr;

  if (hostStr.includes('www.toolimg.online')) {
    shouldRedirect = true;
    newHost = 'toolimg.online';
  }

  if (protocolStr === 'http' && hostStr.includes('toolimg.online')) {
    shouldRedirect = true;
  }
